...
public class Util_sms {
	...
	private void sendSMS(HttpServletRequest request, HttpServletResponse response, String tel, String contents,
		...
		try {
			...
			wr.flush();
			/*
			 * // 결과값 얻기 BufferedReader rd = new BufferedReader(new
			 ...
			 * tmpMsg.split(","); String Result= rMsg[0]; //발송결과 String Count= ""; //잔여건수
			 * if(rMsg.length>1) {Count= rMsg[1]; }
			 */
			/*
			 * //발송결과 알림 if(Result.equals("success")) { alert = "성공적으로 발송하였습니다."; alert +=
			 * " 잔여건수는 "+ Count+"건 입니다."; } else if(Result.equals("reserved")) { alert =
			 * "성공적으로 예약되었습니다"; alert += " 잔여건수는 "+ Count+"건 입니다."; } else
			 * if(Result.equals("3205")) { alert = "잘못된 번호형식입니다."; } else { alert =
			 * "[Error]"+Result; }
			 */
